...
# The classifier learns and predicts on all subjects at the same time.
...
def classify_allsubs(data, classifier, classifier_name, nSamples, nFeatures, nClasses, labels, balancing, time_switch):
    ...
    for itime in range(nSamples):
        print(f"{classifier_name} | allsubs | itime: {itime} / {nSamples-1}")
        ...
        for train_ix, test_ix in kfold.split(X, y):
            ...
            meanSamples = np.mean([Counter(y_train_orig).get(0), Counter(y_train_orig).get(1),
                                   Counter(y_train_orig).get(2), Counter(y_train_orig).get(3)], dtype=int)
            rus = RandomUnderSampler(sampling_strategy={0: meanSamples, 2: meanSamples}, random_state=0)
            X_train_temp, y_train_temp = rus.fit_resample(X_train_orig, y_train_orig)

            if balancing == 'smote':
                overSmote = SMOTE(sampling_strategy={1: meanSamples, 3: meanSamples}, random_state=0)
                X_train, y_train = overSmote.fit_resample(X_train_temp, y_train_temp)
            elif balancing == 'ros':
                ros = RandomOverSampler(sampling_strategy={1: meanSamples, 3: meanSamples}, random_state=0)
            ...
            auc_kfold += roc_auc_score(y_test_orig, y_pred_prob, average='weighted', multi_class='ovr')
            auc_std_kfold.append(roc_auc_score(y_test_orig, y_pred_prob, average='weighted', multi_class='ovr'))
            precision_kfold += precision_score(y_test_orig, y_pred, average='weighted', zero_division=0)
            recall_kfold += recall_score(y_test_orig, y_pred, average='weighted', zero_division=0)
